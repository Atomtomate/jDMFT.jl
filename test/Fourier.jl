@testset "τIntegrate" begin
    ...
    data = τGrid .^ 2
    fτ = τFunction(data, 1.0, τGrid, τWeights, [0.0, 0.0])
    ...
    rG, rW = jDMFT.riemann(3.13, 5.67, 100)
    grG, grW = jDMFT.gaussradau(3.13, 5.67, 100)
...
@testset "FT Tests" begin
    τGrid, τWeights = jDMFT.riemann(0,β-1/length(GW.data),length(GW.data))
    G_τ = ω_to_τ(GW, τWeights, τGrid)
    G2 = τ_to_ω(G_τ, νnGrid)
    @test maximum(abs.((GW.data .- G2.data) ./ GW.data)) < 1e-8
    @test real(G2.data[1] .* νnGrid[1]) ≈ 1.0 rtol=0.01
end
